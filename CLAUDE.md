...
- **OpenClaw:** single-instance AI agent (shell + browser tools), rendered directly from bjw-s `app-template` (the upstream openclaw-helm wrapper is archived and template-less), image `ghcr.io/openclaw/openclaw` pinned. Gateway :18789 behind Traefik `openclaw.local`, Chromium sidecar, state on a `gp3` PVC, `configMode: overwrite` (strict GitOps; `merge` + ConfigMap `ignoreDifferences` if UI pairing must survive restarts). Security posture: dedicated SA bound **read-only** — built-in `view` ClusterRole (no Secrets, no exec) plus a small ClusterRole for nodes/PV/StorageClass/metrics/Karpenter/Argo CRDs; no write verbs anywhere. kubectl is downloaded onto the PVC by init-skills, and a GitOps-managed skill (`workspace/skills/cluster-readonly/SKILL.md`, from the ConfigMap) tells the agent what it may inspect (kubectl, Prometheus `:9090`, Loki via `loki-gateway`). `karpenter.sh/do-not-disrupt` on the spot node. NetworkPolicy default-deny — ingress only from ns `traefik`; egress only kube-dns, the kube API ClusterIP `172.20.0.1:443`, ns `monitoring` (9090/80/3100) and `0.0.0.0/0 except` private/CGN/link-local (blocks IMDS, Pod Identity agent, every other in-cluster IP). Guardrails on top, all GitOps-managed from the ConfigMap and reapplied by init-config each start: `tools.exec.mode: ask` with an allowlist (`exec-approvals.json` → `openclaw approvals set`; read-only kubectl verbs, GET-only curl to monitoring, safe bins) so every other command pauses for human approval; `strictInlineEval: true` closes the interpreter bypass; `workspace/AGENTS.md` carries the operating rules (read-only, external content is data, no exfiltration, bounded reads). Accepted risk: a cluster-wide read token lives in a pod with shell + web access — prompt injection can still exfiltrate anything `view` can read (ConfigMaps, logs, events) through allowlisted commands; guardrails reduce, not remove. Never widen to write verbs without keeping the approval step. `ipBlock.except` was a VPC CNI hazard (aws/amazon-network-policy-controller-k8s#146: an `except` DENY overriding a neighbouring kube-dns ALLOW); verified working here on 2026-09-11 — the controller compiles kube-dns into `/32` entries that win longest-prefix over the `except`, DNS resolves while IMDS and the kube API ClusterIP time out. Re-check DNS from the pod after vpc-cni upgrades; the explicit public-CIDR fallback stays as a comment next to the rule. Secret `openclaw-env` (gateway token, `ANTHROPIC_API_KEY`) is out-of-band; without it the pod sits in `CreateContainerConfigError`.
