...
- **Istio** two ApplicationSets, gated by `enable_istio`, parallel to (not via) Traefik:
  - **istio-base** (`sync-wave: "0"`): chart `base` from `https://istio-release.storage.googleapis.com/charts`, pinned `1.30.1`. Installs Istio CRDs + cluster resources into `istio-system`. `helm.values: defaultRevision: default` binds the `ValidatingWebhookConfiguration` to the unrevisioned (`default`) istiod so CR validation is active. `ServerSideApply=true` + `CreateNamespace=true` in syncOptions — CRDs are large (annotation exceeds client-side apply limit, same reason as GPU-operator `ClusterPolicy` / argo-workflows).
  - **istiod** (`sync-wave: "1"`): chart `istiod`, same repo/version. Multi-source — chart + `$values` ref to `argocd/helm-values/istiod/values.yaml`. Single replica, pinned to the karpenter system NG (`karpenter.sh/controller`), envoy `accessLogFile: /dev/stdout` so logs land in Alloy→Loki. `ServerSideApply=true`. `ignoreDifferences` on `Mutating`/`ValidatingWebhookConfiguration` `.webhooks[].clientConfig.caBundle` — istiod self-injects the caBundle at runtime, otherwise permanent ArgoCD drift (same pattern as piraeus/jupyterhub). **Control plane only** — no ingress gateway yet (the `gateway` chart + `Gateway`/`VirtualService` CRs are deferred to the llm-d / kubeflow track). Workload namespaces opt in via `istio-injection=enabled` (or `istio.io/rev`) — not set by these charts.
