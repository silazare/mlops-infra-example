...
Multi-model LLM serving with **KV-cache-aware pod routing** (llm-d), gated by `enable_llm_d`, namespace `llm-d`. Long-form plan + decisions in [LLM_PLAN.md](LLM_PLAN.md). **Status: M1 + T6 + T10 + T8/T9 done (M2+M3, 2026-07-17)** — **two models serve simultaneously** (Qwen2.5-0.5B + Phi-4-mini-instruct), each through its own **Istio Gateway** edge with EPP smart routing, **unified behind one LiteLLM endpoint** `http://litellm.local` (Traefik door, master-key auth). Per-model direct access stays via gateway/epp port-forward; per-model internal ALB direct door = T13. Remaining: T11 scale-to-zero (M4), T12 observability, T13 ingress hardening.
