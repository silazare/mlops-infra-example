...
Multi-model LLM serving with **KV-cache-aware pod routing** (llm-d), gated by `enable_llm_d`, namespace `llm-d`. Long-form plan + decisions in [LLM_PLAN.md](LLM_PLAN.md). **Status: M1 + T6 done** — Qwen2.5-0.5B serving + smart routing verified, and the **Istio Gateway edge works end-to-end** (streaming chat via `port-forward svc/<rn>-gateway-istio`, 2026-06-29). No external door yet (ALB = T13) and no LiteLLM aggregator (T8).
