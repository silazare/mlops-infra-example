...
- **`gp3` StorageClass** — Prometheus/Loki/Grafana PVCs depend on it. Putting it under ArgoCD creates a bootstrap deadlock (apps requiring gp3 hang waiting for the storage-classes Application to sync first).
...
argocd/applications/              # GitOps — discovered recursively by the root Application
  core/                           # platform-layer components
    ...
    kube-prometheus-stack.yaml    # ApplicationSet, gated enable_monitoring; prometheus-operator + Prometheus + Grafana + kube-state-metrics + node-exporter (chart + $values, SSA)
    grafana-loki.yaml             # ApplicationSet `loki`, gated enable_monitoring; multi-source Loki single-binary (filesystem) + Promtail charts + $values
    nvidia-gpu-operator.yaml      # ApplicationSet, GPU Operator chart (driver + toolkit + device-plugin + dcgm + NFD/GFD); 3rd source applies argocd/manifests/nvidia-gpu-operator (custom dcgm metric ConfigMap)
...
argocd/helm-values/               # static Helm values pulled via multi-source $values ref
  ...
  kube-prometheus-stack/values.yaml # Grafana (admin pass, Traefik ingress grafana.local, Loki datasource uid=loki, dashboard 15141) + Prometheus/Alertmanager (gp3 PVCs, Traefik ingresses, retention 1d) + default rules
  grafana-loki/
    loki-values.yaml              # SingleBinary, filesystem, canary/test/chunksCache/resultsCache disabled
    promtail-values.yaml          # DaemonSet -> loki-gateway push; tolerate-everything (tainted pools); systemd-journal scrape
  nvidia-gpu-operator/values.yaml # driver pinned to nvcr.io tag, toolkit on, MIG/vGPU off, dcgm-exporter + NFD/GFD on, dcgmExporter.config.name=dcgm-metrics-custom (custom metric set)
  istiod/values.yaml              # single replica, pinned to system node group, envoy accessLog -> stdout (Promtail/Loki)
...
- [prometheus-community/kube-prometheus-stack chart 83.x](https://github.com/prometheus-community/helm-charts) — prometheus-operator + Prometheus + Alertmanager + Grafana + KSM + node-exporter
- [grafana/loki Helm chart 6.x](https://github.com/grafana/loki) — single-binary mode
- [grafana/promtail Helm chart 6.x](https://github.com/grafana/helm-charts) — log shipping DaemonSet
...
IAM role ARNs are **not** published as annotations — Pod Identity associations (see [iam.tf](terraform/iam.tf) and the EBS CSI inline `pod_identity_association` in [eks.tf](terraform/eks.tf)) wire SAs to IAM roles at the AWS API level, so Helm values never need the role ARN.
...
|---|---|---|
| `enable_monitoring` | metrics + logs stack | kube-prometheus-stack, loki (Loki + Promtail) |
...
- **kube-prometheus-stack** ApplicationSet (gated `enable_monitoring`, ns `monitoring`, SSA — CRDs exceed client-side apply): prometheus-operator + Prometheus + Alertmanager + Grafana + kube-state-metrics + node-exporter in one chart (83.7.0). Ported from [argocd-infra-example](../argocd-infra-example/). Prometheus: 1d retention, 5Gi gp3 PVC, Traefik ingress `prometheus.local`; Alertmanager: 1Gi gp3, `alertmanager.local`; kubelet ServiceMonitor with cAdvisor/probes/resource over https (EKS). Discovery is ServiceMonitor/PodMonitor-based (prometheus-operator CRDs) — no hand-written scrape configs.
- **Grafana** (inside kube-prometheus-stack): Prometheus default datasource via sidecar (`uid: prometheus`), Loki datasource `uid: loki` → `http://loki-gateway.monitoring.svc.cluster.local/`, Traefik ingress `grafana.local`, 1Gi gp3 persistence, dashboard `15141` (Loki kubernetes logs) via gnetId.
- **Loki** ApplicationSet (`loki`, gated `enable_monitoring`): multi-source — Loki chart (SingleBinary, filesystem storage on 10Gi gp3 PVC, no S3; `lokiCanary`/`test`/`chunksCache`/`resultsCache` disabled) + **Promtail** chart (DaemonSet, pushes to `loki-gateway`). Promtail values add tolerate-everything tolerations — the cluster's node pools are tainted (karpenter system NG, GPU pools, ubuntu, linstor-storage) and the chart default would skip those nodes; plus a systemd-journal scrape (hostPath `/var/log/journal`).
- **NVIDIA GPU Operator** ApplicationSet: gpu-operator chart with driver-DaemonSet + toolkit + device-plugin + dcgm-exporter + NFD + GFD. Driver image version pinned in values (chart default tag often missing for `-amzn2023` on nvcr.io; the `ubuntu-gpu` NodePool uses Ubuntu where coverage is reliable). Tolerates `nvidia.com/gpu=true:NoSchedule`. `ServerSideApply=true` in syncOptions because the `ClusterPolicy` CRD has annotations that exceed kubectl client-side apply limit. **Custom dcgm metric set:** the image-default dcgm-exporter csv OMITS the productivity signals (`DCGM_FI_PROF_SM_ACTIVE`/`SM_OCCUPANCY`) plus `FB_TOTAL`/`POWER_MGMT_LIMIT`/throttle+violation counters, so a `ConfigMap dcgm-metrics-custom` ([argocd/manifests/nvidia-gpu-operator/dcgm-metrics.yaml](argocd/manifests/nvidia-gpu-operator/dcgm-metrics.yaml)) ships the full set and is wired via `dcgmExporter.config.name` in values; the operator mounts it and starts the exporter with `-f` (env `DCGM_EXPORTER_CONFIGMAP_DATA=gpu-operator:dcgm-metrics-custom`). The ConfigMap is a **3rd source** (`path:`) on the ApplicationSet — the upstream chart has no inline-csv option. **csv is strict: exactly 3 comma-separated fields per line (`FIELD, type, help`) — a comma inside the help text fails the exporter with `record on line N: wrong number of fields`.** **Scraping:** `dcgmExporter.serviceMonitor.enabled: true` — picked up by kube-prometheus-stack. ⚠ This couples gpu-operator to the prometheus-operator CRDs: with `enable_monitoring=false` the ServiceMonitor CRD is absent and the gpu-operator app fails to sync — flip the serviceMonitor off if monitoring is ever disabled. The kps values set `*SelectorNilUsesHelmValues: false` so Prometheus picks up ALL ServiceMonitors/PodMonitors cluster-wide, not only release-labelled ones (needed here and for future vLLM/EPP monitors, T12).
...
|---|---|---|---|---|---|
| `default` | AL2023 (alias `al2023@latest`) | t-family 2–8 vCPU | spot | none | General workloads (Prometheus, Loki, Grafana, BuildKit Job, etc.) |
...
- **Metrics** `controller.metricsConfig.enabled: true` (`:8080/metrics`); `serviceMonitor.enabled: false` — flip to `true` to scrape via kube-prometheus-stack (prometheus-operator CRDs are present when `enable_monitoring=true`).
...
2. **Artifacts** — `artifactRepository.s3` (bucket/endpoint/region) + an ExternalSecret `argo-artifacts` in the workflow ns (`argo`) via `extraObjects`. On EKS prefer Pod Identity (`useSDKCreds: true`, drop static keys) + an S3 bucket + role in Terraform.
...
Multi-model LLM serving with **KV-cache-aware pod routing** (llm-d), gated by `enable_llm_d`, namespace `llm-d`. Long-form plan + decisions in [LLM_PLAN.md](LLM_PLAN.md). **Status: COMPLETE for sandbox scope (M0–M4, 2026-07-19)** — **two models serve simultaneously** (Qwen2.5-0.5B ×2 replicas + Phi-4-mini-instruct), each through its own **Istio Gateway** edge, **unified behind one LiteLLM endpoint** `http://litellm.local` (Traefik door, master-key auth). **EPP prefix-cache affinity PROVEN** (same-prefix load sticks to one pod: 59/20 spread, hit-rate confirmed via `vllm:prefix_cache_hits_total`); scale-to-zero verified (M4). T13 (ALB direct door / TLS / Route53) dropped by decision; T14 (Redis precise routing, M5) = nice-to-have someday. Cost note: qwen can go back to `replicas: "1"` + `modelCacheEnabled: "true"` any time.
...
- **Layer 1 — model selection (built, verified):** LiteLLM maps an alias (`qwen-0.5b` / `phi-4-mini`) → that model's gateway Service via cluster DNS (D1 Variant A). [litellm.yaml](argocd/applications/mlops/litellm.yaml) (wave 2) + [values.yaml](argocd/helm-values/litellm/values.yaml). **DB-less sandbox mode:** `db.deployStandalone: false` + `migrationJob.enabled: false` (chart defaults would pull bitnami Postgres + a Prisma migration Job). Master key **hardcoded in values** (sandbox — non-production secrets in git; the chart renders the `litellm-masterkey` Secret from it deterministically; leaving `masterkey` unset makes the chart autogenerate per render ⇒ ArgoCD drift; prod = `masterkeySecretName` + External Secrets). External door = chart-native Traefik Ingress `litellm.local`. **Auth gotcha:** a wrong master key does NOT 401 — DB-less LiteLLM falls through to a virtual-key DB lookup → `400 No connected db.`; check the key first, not the DB config.
...
- **Add a model = ONE entry** in the recipe ApplicationSet `list.elements`. Router base/optimized-baseline values are vendored verbatim in `values.yaml`; our deltas are marked `# OVERRIDE`. Per-model inline (in `llm-d-recipe.yaml`): `gpuMemUtil` + `hfTokenSecret` (modelserver) and `router.proxy.presets.envoy.configMap.name` **and** `router.proxy.presets.envoy.volumes` set to `<resourceName>-envoy` (router) — the router chart defaults the bundled-Envoy ConfigMap to a **fixed name `envoy`** which **collides across models in the same ns** (ArgoCD `SharedResourceWarning` + ownership flapping); every other router object is release-prefixed. **v0.9.0 gotcha:** the envoy preset names the CM in TWO independent spots — `configMap.name` (the object created) and `volumes[].configMap.name` (the sidecar mount ref, a separate hardcoded literal `envoy`). Overriding only `configMap.name` renames the created CM but leaves the volume pointing at `envoy` → EPP pod stuck `ContainerCreating` on `MountVolume.SetUp failed … configmap "envoy" not found`. Must respec the whole `config` volume (lists are replaced, not merged) so both point at `<resourceName>-envoy`.
...
# Inspect Prometheus targets / config (or prometheus.local via Traefik)
kubectl -n monitoring port-forward svc/kube-prometheus-stack-prometheus 9090:9090
# open http://localhost:9090/targets

# Query Loki directly (or loki.local via Traefik)
kubectl -n monitoring port-forward svc/loki-gateway 8090:80
curl http://localhost:8090/loki/api/v1/labels
